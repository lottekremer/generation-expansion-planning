...
insample <- read_csv("C:/Users/kremerlaa/OneDrive - TNO/Documents/gep/generation-expansion-planning/case_studies/distribution/results/insample_distribution.csv")
outsample <- read_csv("C:/Users/kremerlaa/OneDrive - TNO/Documents/gep/generation-expansion-planning/case_studies/distribution/results/outsample_distribution.csv")

# Bundling insample and outsample data
insample$type <- "insample"
outsample$type <- "outsample"
combined_output <- rbind(insample, outsample)
...
categories <- c("softmax", "centered", "closemixed", "close")

############### Plots relative regret ########################################
for (cat in categories) {
  p <- ggplot(output_final %>% filter(data == cat), aes(x = num_periods, y = cost_increase, 
  ...
  show(p)
  save_plot(p, paste0('C:/Users/kremerlaa/OneDrive - TNO/Documents/gep/Results/Distribution/', cat, '_cost.pdf'), 0.4)}

############### Plots loss of load ########################################
for (cat in categories) {
  p <- ggplot(output_final %>% filter(data == cat), aes(x = num_periods, y = loss_ct_increase, 
                                                        colour = clustering, fill = clustering, linetype = clustering)) +
    geom_line(linewidth = 0.4) +
    geom_point(size = 0.4) +
    labs(
    ...
    geom_ribbon(data = output_quantiles_loss %>% filter(data == cat),
                mapping = aes(x = num_periods, ymin = q25, ymax = q75, fill = clustering), 
                alpha = 0.2, inherit.aes = FALSE)+
    coord_cartesian(xlim = c(3, 41), 
    ...
    facet_grid(rows = vars(distance), cols = vars(type),
               labeller = labeller(
                 ...
                 type = c("insample" = "In-sample evaluation", "outsample" = "Out-of-sample evaluation"))) +
    scale_color_manual(values = my_colors, 
                       labels = c("convex_hull" = "Greedy Convex Hull", 
                                  "k_means" = "K-means", 
                                  "k_medoids" = "K-medoids",
                                  "conical_hull" = "Greedy Conical Hull")) +
    scale_fill_manual(values = my_colors, 
                      labels = c("convex_hull" = "Greedy Convex Hull", 
                                 "k_means" = "K-means", 
                                 "k_medoids" = "K-medoids",
  ...
  show(p)
  save_plot(p, paste0('C:/Users/kremerlaa/OneDrive - TNO/Documents/gep/Results/Distribution/',cat,'_loss.pdf'), 0.4)}

 ############ Table ############
library(writexl)
write_xlsx(output_final, "C:/Users/kremerlaa/OneDrive - TNO/Documents/gep/Results/Distribution/output_final.xlsx")
output_table <- output_final %>% filter(distance == "CosineDist", num_periods %in% c(3,5,7,9,11)) %>% select(data, num_periods, clustering, cost_increase, loss_ct_increase, type) %>% filter(data == "softmax")