...
###################### RUNTIME PLOTS #################################
bootstrap_ci_loss_ct <- function(df, metric_col = "total_loss_ct", R = 1000) {
...
bootstrap_results_loss_ct <- insample_summarized %>%
  filter(clustering %in% c("k_medoids", "k_means")) %>%
  group_by(method, clustering, num_periods) %>%
  group_modify(~ {
    ci_result <- bootstrap_ci_loss_ct(.x, metric_col = "total_loss_ct", R = 1000)
    tibble(
...
loss_ct_methods_summary <- insample_summarized %>%
  filter(clustering %in% c("convex_hull", "conical_bounded")) %>%
  group_by(method, num_periods, clustering) %>%
  summarise(mean_total_loss_ct = mean(total_loss_ct, na.rm = TRUE), .groups = "drop")


results_loss_ct_mean <- bootstrap_results_loss_ct %>%
  select(method, num_periods, clustering, mean_total_loss_ct)

results_loss_ct_mean <- rbind(results_loss_ct_mean, loss_ct_methods_summary)

results_loss_ct_all <- results_loss_ct_mean %>%
  left_join(
    bootstrap_results_loss_ct,
  ...
  select(
    ...
    num_periods,
    clustering,
    mean_total_loss_ct = mean_total_loss_ct.x,
    ci_lower = ci_lower,
    ci_upper = ci_upper
  )
...
p_loss_ct <- ggplot() +
  ...
  labs(
    title = "Number of time steps with loss of load",
    x = "Representative Period Selection Method",
    y = "Mean time steps with loss of load",
...
save_plot(p_loss_ct, paste0('C:/Users/kremerlaa/OneDrive - TNO/Documents/gep/Results/Europe/total_loss_ct.pdf'), 0.4)
