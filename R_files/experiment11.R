...
# Ensure clustering is a factor with the correct levels
...
for (cat in categories) {
  ...
  labs(
    ...
    y = "Cost Increase (%)", 
    title = paste("Cost Increase % for different number of Representative Periods - ", cat_names[cat]),
    ...
    facet_grid(rows = vars(distance), cols = vars(type),
      ...
      type = c("insample" = "In-sample evaluation", "outsample" = "Out-sample evaluation"))) +
  ...
  show(p)
  save_plot(p, paste0('C:/Users/kremerlaa/OneDrive - TNO/Documents/gep/Results/', cat, '_cost.pdf'), 0.4)}
...
for (cat in categories) {
  ...
  show(p)
  save_plot(p, paste0('C:/Users/kremerlaa/OneDrive - TNO/Documents/gep/Results/',cat,'_loss.pdf'), 0.4)}


 ############ Table ############
output_table <- output_final %>% filter(distance == "CosineDist") %>% select(data, num_periods, clustering, cost_increase, loss_ct_increase) %>% 
