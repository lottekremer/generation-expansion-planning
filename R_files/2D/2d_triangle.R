...
df_triangle <- read_csv("C:/Users/kremerlaa/OneDrive - TNO/Documents/gep/generation-expansion-planning/case_studies/2d/results/triangle.csv")
...
########################### Test #############################################
takes <- c(6, 11, 16, 21, 26, 31, 36, 41, 46, 51, 56, 61)
take_data <- points_wide %>%
  filter(take %in% takes, num_periods == 3, clustering == "k_means", distance == "CosineDist") 

# Renumber based on matching position
take_data <- take_data %>%
  mutate(
    take = match(take, takes)
  ) %>%
  mutate(
    take = paste0("Triangle ", take)
  )

# Set correct factor levels for proper ordering
take_data$take <- factor(take_data$take, levels = paste0("Triangle ", 1:14))

triangle_data <- take_data %>%
  arrange(take, representative_period) %>%  # important to arrange by take and period
  group_by(take) %>%
  summarise(
    wind = c(wind, wind[1]),  # close the triangle (repeat first point)
    demand = c(demand, demand[1]),
    .groups = "drop"
  ) %>%
  unnest(cols = c(wind, demand))  # Make it flat again

# Create blue gradient palette
blue_gradient <- colorRampPalette(brewer.pal(9, "Blues"))(length(takes))
take_palette <- setNames(blue_gradient, paste0("Triangle ", takes))

# Now plot with facets
p <- ggplot(initial, aes(x = wind, y = demand))  +
  geom_point(size = 0.1) +
  geom_point(data = outer, aes(x = wind, y = demand), size = 0.1, colour = "grey")+
  geom_polygon(data = triangle_data, aes(x= wind, y = demand, group = take, fill = take), alpha = 0.4, color = "black", linewidth = 0.3) +
  thesis_theme() +
  scale_fill_manual(values = colorRampPalette(RColorBrewer::brewer.pal(9, "Blues"))(length(takes))) +
  labs(
    x =  "Wind availability",
    y = "Demand (scaled)", 
  ...
  coord_cartesian(xlim = c(0, 1), 
                  ylim = c(0, 1.5))  + facet_wrap(~take, ncol = 4) + guides(fill = "none")

show(p)

save_plot(p, paste0('C:/Users/kremerlaa/OneDrive - TNO/Documents/gep/Results/2D/example_triangles.pdf'), 0.65)
...
save_plot(p, paste0('C:/Users/kremerlaa/OneDrive - TNO/Documents/gep/Results/2D/example_triangle.pdf'), 0.6)
