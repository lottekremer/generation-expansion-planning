...
library(ggplot2)
library(dplyr)
combined_output <- read_csv("C:/Users/kremerlaa/OneDrive - TNO/Documents/gep/generation-expansion-planning/case_studies/stylized_EU/res/results_csv/combined_output_newloss.csv")
...
output <- filter(output, num_periods == 50)

# Plot the results
ggplot(output)+geom_(mapping = aes(x = speedup, y = average_regret, colour = clustering, shape = blended), size = 2) + 
