...
println("Debugging fit_rep_period_weights!() ...")
  ...
  for period in periods
    ...
    println("New value it is represented by:", rp_matrix * x)
...
The arguments:
  ...
  - other arguments control the projected subgradient method; they are passed
    through to `TulipaClustering.projected_subgradient_descent!`.
"""
function fit_rep_period_weights_2(
  clustering_result;
  weight_type::Symbol = :dirac,
  tol::Float64 = 10e-3,
  args...,
...
df = DataFrame(
  ...
  value = [1.0, 2.0, 2.0, 1.0, 2.0, 2.0, 2.0, 3.0, 3.0, 1.0, 3.0, 2.0, 3.0, 3.0]
)
