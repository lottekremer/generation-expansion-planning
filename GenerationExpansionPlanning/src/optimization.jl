export run_experiment, run_rp, run_fixed_investment
...
function run_experiment(data::ExperimentData, optimizer_factory)::ExperimentResult
    ...
    dt = @elapsed begin
        ...
        ramping = @expression(model, [n ∈ N, g ∈ G, p ∈ P, t ∈ T, s ∈ S; t > minimum(T) && (n, g) ∈ NG],
...
function run_rp(data::RepData, optimizer_factory)::ExperimentResult
    # 1. Extract data into local variables
    @info "Reading the sets"
    N = data.locations
    G = data.generation_technologies
...
function run_fixed_investment(data::FixedData, optimizer_factory)::ExperimentResult
    # 1. Extract data into local variables
    @info "Reading the sets"
    N = data.locations
    G = data.generation_technologies
    ...
    @info "Converting dataframes to dictionaries"
    demand = dataframe_to_dict(data.demand, [:location, :timestep, :scenario, :period], :demand)
    generation_availability = dataframe_to_dict(data.generation_availability, [:location, :technology, :timestep, :scenario, :period], :availability)
