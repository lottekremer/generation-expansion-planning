...
generation_values = (min_wind + max_wind) / 2 .+ (max_wind - min_wind) / 4 .* cos.(t)
demand_values = (min_demand + max_demand) / 2 .+ (max_demand - min_demand) / 2 .* sin.(t)

demand = DataFrame(period=1:100, timestep=1, scenario=1900, location="GER", demand=demand_values)
generation_availability = DataFrame(period=1:100, timestep=1, scenario=1900, location="GER", availability=generation_values, technology="WindOn")
println("Demand: ", demand)
println("Generation Availability: ", generation_availability)

CSV.write("./case_studies/2d/inputs/demand.csv", demand)
CSV.write("./case_studies/2d/inputs/generation_availability.csv", generation_availability)
CSV.write("./case_studies/2d/inputs/generation.csv", generation)
CSV.write("./case_studies/2d/inputs/transmission_lines.csv", transmission_lines)
